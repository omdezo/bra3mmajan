          ...
          <div className="font-medium text-white flex items-center gap-2">
            ...
            {row.pptUrl && (
              <span className="text-amber-400 text-xs">
                {row.pptUrl.includes('canva.com') ? '🎨' : row.pptUrl.includes('drive.google.com') ? '📁' : '📊'}
              </span>
            )}
            {row.imageUrl && <span className="text-emerald-400 text-xs" title="صورة مصغرة مخصصة">🖼️</span>}
          </div>
  ...
  return (
    ...
    >
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}
        title={editId ? 'تعديل الكنز' : 'إضافة كنز جديد'} size="lg">
        <div className="space-y-4" dir="rtl">
          ...
          <FormField label="المحتوى التفصيلي">
            <Textarea value={form.content} onChange={e => upd('content', e.target.value)} rows={3} placeholder="معلومات تفصيلية..." />
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField label="الترتيب" hint="رقم أصغر = يظهر أولاً">
              <Input type="number" value={String(form.order)} onChange={e => upd('order', Number(e.target.value))} />
            </FormField>
          </div>
          ...
          <div className="border border-white/10 rounded-xl p-4 space-y-3">
            <p className="text-slate-200 text-sm font-semibold flex items-center gap-2">
              <span>🖼️</span><span>صورة مصغرة (Thumbnail)</span>
            </p>
            <FormField label="رابط الصورة" hint="تظهر على بطاقة المنوعات — اتركها فارغة ليتم جلبها تلقائياً من الرابط">
            ...
            {form.imageUrl && (
              <div className="relative w-full h-32 rounded-xl overflow-hidden border border-white/10 bg-slate-800">
                ...
                <button
                  ...
                  className="absolute top-2 left-2 w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full text-xs flex items-center justify-center font-bold transition"
                >✕</button>
              </div>
            )}
          </div>

          <FormField label="رابط الصوت" hint="MP3 أو رابط مباشر">
            <Input value={form.audioUrl} onChange={e => upd('audioUrl', e.target.value)} placeholder="https://..." />
          </FormField>
          <FormField label="رابط الفيديو" hint="MP4 أو رابط مباشر">
            <Input value={form.videoUrl} onChange={e => upd('videoUrl', e.target.value)} placeholder="https://..." />
          </FormField>

          {/* ── PPT/Embed ── */}
          <div className="border-t border-white/10 pt-4">
            <p className="text-slate-300 text-sm font-semibold mb-3 flex items-center gap-2">
              <span>📊</span><span>عرض تقديمي (Canva / PowerPoint / PDF)</span>
            </p>
            <FormField label="" hint="يُعرض داخل الموقع مباشرةً بدون تحميل">
              <PPTUploader value={form.pptUrl || undefined} onChange={url => upd('pptUrl', url ?? '')} />
            </FormField>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={handleSave} disabled={saving}
              className="flex-1 bg-amber-600 hover:bg-amber-700 text-white rounded-xl py-2.5 font-medium transition disabled:opacity-60">
